    ...
    <?php
        if ( !empty($gallery_attachments) ) {
            ...
            foreach ($gallery_attachments as $gallery_attachment)
            {
                ...
                echo '<li>' . apply_filters( 'propertyhive_single_property_image_html', sprintf( '<a href="%s" class="propertyhive-main-image" title="%s" data-fancybox="gallery">%s</a>', $image_link, $image_title, $image ), $post->ID ) . '</li>';
            }

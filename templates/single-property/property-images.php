...
<div class="images">
    ...
    <?php
        if ( isset($images) && is_array($images) && !empty($images) ) {
            ...
            {
                echo '<li>' . apply_filters( 'propertyhive_single_property_image_html', sprintf( '<a href="%s" class="propertyhive-main-image" title="%s" data-fancybox="gallery-' . $post->ID . '">%s</a>', esc_attr( $image['url'] ), esc_attr( $image['title'] ), $image['image'] ), $post->ID ) . '</li>';
            }

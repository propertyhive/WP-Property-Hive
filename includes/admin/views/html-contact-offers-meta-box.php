    ...
    <?php
        if ( $offers_query->have_posts() )
        {
            while ( $offers_query->have_posts() )
            {
                ...
                $column_data = array(
                    'date' => '<a href="' . $edit_link . '" target="' . apply_filters('propertyhive_subgrid_link_target', '') . '">' . date("jS F Y", strtotime($the_offer->_offer_date_time)) . '</a>',
        ...
        else
        {
            ?>
            <tr class="no-items">
                <td class="colspanchange" colspan="<?php echo count($columns); ?>"><?php echo __( 'No offers found', 'propertyhive' ); ?></td>
            </tr>
            <?php
        }
        wp_reset_postdata();

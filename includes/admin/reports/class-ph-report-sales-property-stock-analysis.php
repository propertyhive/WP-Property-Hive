...
/**
 * PH_Report_Sales_Property_Stock_Analysis
 *
 * @author      PropertyHive
 * @category    Admin
...
<div class="chart-tabs">
	<ul>
		<li<?php if ( $report_type == 'averages' ) { echo ' class="active"'; } ?>><a href="admin.php?page=ph-reports&tab=properties&report=sales_property_stock_analysis">Averages</a></li>
		<li<?php if ( $report_type == 'totals' ) { echo ' class="active"'; } ?>><a href="admin.php?page=ph-reports&tab=properties&report=sales_property_stock_analysis&report_type=totals">Totals</a></li>
	</ul>
</div>
<div class="chart-with-sidebar">
	...
	<div class="chart-sidebar">
		...
		<p>This report will look at all the properties currently on the market and display the average price or number of bedrooms (metric two) for each metric one chosen.</p>
	...
	var options = { 
		...
		yaxis: {  
    		...
    		tickFormatter: function (v, axis) {
		        //return jQuery.formatNumber(v, { format: "#,###", locale: "us" });
		        return v;
		    },
		},
		xaxis: {  
...
<?php
	...
	else
	{
		if ( $metric_one == 'price' )
		{
			foreach ( $price_ranges as $price_range )
			{
				$labels[$price_range['from']] = '£' . number_format($price_range['from']) . ' - £' . number_format($price_range['to']);
			}
		}
	...
	jQuery(document).ready(function($)
	{
		...
		var options = { 
			...
			yaxis: {  
        		...
        		tickFormatter: function (v, axis) {
			        //return jQuery.formatNumber(v, { format: "#,###", locale: "us" });
			        return <?php if ($metric_one == 'price') { echo '"£" + '; } ?>v;
			    },
    		},
    		xaxis: {  
        		//axisLabel: "Average Price (£)",
        		...
        		tickFormatter: function (v, axis) {
			        //return jQuery.formatNumber(v, { format: "#,###", locale: "us" });
			        return <?php if ($metric_two == 'price') { echo '"£" + '; } ?>v;
			    },
    		}
		};
	...
	jQuery.fn.useTooltip = function () {
		jQuery(this).bind("plothover", function (event, pos, item) 
		{
			if (item) {
				...
				jQuery("#tooltip").html(item.series.label + " of <?php if ($metric_two == 'price') { echo '£'; } ?>" + x<?php if ($metric_two == 'bedrooms') { echo ' + " bedrooms"'; } ?>)
					.css({top: item.pageY+5, left: item.pageX})
					.fadeIn(200);
			} else {
				jQuery("#tooltip").hide();

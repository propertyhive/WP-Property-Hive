 ...
 */
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound -- Legacy public global class PH_Meta_Box_Property_Floorplans; preserving the existing PH_* class name is required for plugin and extension compatibility.
class PH_Meta_Box_Property_Floorplans {
    ...
    public static function save( $post_id, $post ) {
        ...
        if ( get_option('propertyhive_floorplans_stored_as', '') == 'urls' )
        {
            ...
            if ( ! isset( $_POST['floorplan_url'] ) || ! is_array( $_POST['floorplan_url'] ) || count( array_filter( $_POST['floorplan_url'], 'is_string' ) ) !== count( $_POST['floorplan_url'] ) ) {
                return;
            }
            $submitted_urls = array_map( 'sanitize_url', wp_unslash( $_POST['floorplan_url'] ) );
            $floorplan_urls = array();
            foreach ( $submitted_urls as $floorplan_url ) {
                if ( $floorplan_url !== '' ) {
                    $floorplan_urls[] = array( 'url' => $floorplan_url );
                }
            }
            update_post_meta( $post_id, '_floorplan_urls', wp_slash( $floorplan_urls ) );
        }
        else
        {
            if ( isset( $_POST['floorplan_attachment_ids'] ) && is_string( $_POST['floorplan_attachment_ids'] ) ) {
                propertyhive_save_media_attachment_list( $post_id, '_floorplans', sanitize_text_field( wp_unslash( $_POST['floorplan_attachment_ids'] ) ) );
            }
        }
    }
}

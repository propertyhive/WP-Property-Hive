        ...
        if ( !empty($applicant_solicitor_contact_id) )
        {
            ...
            $fields = array(
                'name' => array(
                    ...
                    'value' => '<a href="' . get_edit_post_link($applicant_solicitor_contact_id, '') . '">' . esc_html(get_the_title($applicant_solicitor_contact_id) . ( $contact->company_name != '' && $contact->company_name != get_the_title($applicant_solicitor_contact_id) ? ' (' . $contact->company_name . ')' : '' )) . '</a>',
                ),
            ...
            echo '<p class="form-field">
                ...
                <a class="button" href="' . wp_nonce_url( admin_url( 'post.php?post=' . $post->ID . '&action=edit' ), '1', 'remove_applicant_solicitor' ) . '">' . esc_html(__( 'Remove Solicitor', 'propertyhive' )) . '</a>
...
jQuery(document).ready(function($)
{
    ...
    $('#sale_applicant_solicitor_search').keyup(function()
    {
        ...
        if (keyword.length < 3)
        {
            $('#sale_search_applicant_solicitor_results').html('<div style="padding:10px;"><?php echo esc_html__( 'Enter ', 'propertyhive' ); ?>' + (3 - keyword.length ) + '<?php echo esc_html__( ' more characters...', 'propertyhive' ); ?></div>');
        ...
        $.post( '<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) 
        {
            ...
            {
                $('#sale_search_applicant_solicitor_results').html('<div style="padding:10px;"><?php echo esc_html__( 'No results found for ', 'propertyhive' ); ?>\'' + keyword + '\'</div>');
            }
            else
            {

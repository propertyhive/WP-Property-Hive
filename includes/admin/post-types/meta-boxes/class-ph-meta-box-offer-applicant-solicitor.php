 ...
 */
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound -- Legacy public global class PH_Meta_Box_Offer_Applicant_Solicitor; preserving the existing PH_* class name is required for plugin and extension compatibility.
        ...
        if ( !empty($applicant_solicitor_contact_id) )
        {
            ...
            foreach ( $fields as $key => $field )
            {
                echo '<p class="form-field ' . esc_attr($key) . '">
                    ...
                    ';
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Core field values are escaped above before the trusted PHP propertyhive_offer_applicant_solictor_fields filter, which intentionally permits extension HTML.
                echo $field['value'];
                echo '

	...
	public static function output( $post ) {
        ...
        else
        {
            echo '<p class="form-field">
                ...
                <label for="offer_applicant_solicitor_search">' . __('Search Solicitors', 'propertyhive') . '</label>
                
                <span style="position:relative;">

                    <input type="text" name="offer_applicant_solicitor_search" id="offer_applicant_solicitor_search" style="width:100%;" placeholder="' . __( 'Search Existing Contacts', 'propertyhive' ) . '..." autocomplete="false">
    ...
    $('#offer_applicant_solicitor_search').keyup(function()
    {
        ...
        $.post( '<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) 
        {
            ...
            else
            {
                ...
                {
                    $('#offer_search_applicant_solicitor_results ul').append('<li style="margin:0; padding:0;"><a href="' + response[i].ID + '" style="color:#666; display:block; padding:7px 10px; background:#FFF; border-bottom:1px solid #DDD; text-decoration:none;">' + response[i].post_title + '</a></li>');
                }
            }
            $('#offer_search_applicant_solicitor_results').show();
        });

 ...
 */
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound -- Legacy public global class PH_Meta_Box_Sale_Property_Owner_Solicitor; preserving the existing PH_* class name is required for plugin and extension compatibility.
	...
	public static function output( $post ) {
        ...
        if ( !empty($property_owner_solicitor_contact_id) )
        {
            ...
            foreach ( $fields as $key => $field )
            {
                echo '<p class="form-field ' . esc_attr($key) . '">
                    ...
                    ';
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Core field values are escaped above before the trusted PHP propertyhive_sale_property_owner_solictor_fields filter, which intentionally permits extension HTML.
                echo $field['value'];
                echo '
            ...
            echo '<p class="form-field">
                ...
                <a class="button" href="' . esc_url(wp_nonce_url( admin_url( 'post.php?post=' . $post->ID . '&action=edit' ), 'propertyhive-remove_property_owner_solicitor-' . $post->ID, 'remove_property_owner_solicitor' )) . '">' . esc_html(__( 'Remove Solicitor', 'propertyhive' )) . '</a>
        ...
        else
        {
            echo '<p class="form-field">
                ...
                <label for="sale_property_owner_solicitor_search">' . esc_html(__('Search Solicitors', 'propertyhive')) . '</label>
                
                <span style="position:relative;">

                    <input type="text" name="sale_property_owner_solicitor_search" id="sale_property_owner_solicitor_search" style="width:100%;" placeholder="' . esc_attr(__( 'Search Existing Contacts', 'propertyhive' )) . '..." autocomplete="false">
...
jQuery(document).ready(function($)
{
    ...
    $('#sale_property_owner_solicitor_search').keyup(function()
    {
        ...
        if (keyword.length < 3)
        {
            $('#sale_search_property_owner_solicitor_results').html('<div style="padding:10px;"><?php echo esc_html__( 'Enter', 'propertyhive' ); ?> ' + (3 - keyword.length ) + ' <?php echo esc_html__( 'more characters', 'propertyhive' ); ?>...</div>');
        ...
        $.post( '<?php echo esc_url(admin_url('admin-ajax.php')); ?>', data, function(response) 
        {
            ...
            {
                $('#sale_search_property_owner_solicitor_results').html('<div style="padding:10px;"><?php echo esc_html__( 'No results found for', 'propertyhive' ); ?> \'' + keyword + '\'</div>');
            }
            else
            {

<?php
/**
 * Admin functions for the application post type
 *
 * @author    PropertyHive
 * @category  Admin
...
if ( ! class_exists( 'PH_Admin_CPT_Application' ) ) :

/**
 * PH_Admin_CPT_Application Class
 */
class PH_Admin_CPT_Application extends PH_Admin_CPT {
	...
	public function __construct() {
		...
		// Admin Columns
		add_filter( 'manage_edit-application_columns', array( $this, 'edit_columns' ) );
	...
	/**
	 * Check if we're editing or adding a application
	 * @return boolean
	 */
	private function is_editing_application() {
		if ( ! empty( $_GET['post_type'] ) && 'application' == $_GET['post_type'] ) {
			return true;
		}
		if ( ! empty( $_GET['post'] ) && 'application' == get_post_type( (int) $_GET['post'] ) ) {
			return true;
		}
		if ( ! empty( $_REQUEST['post_id'] ) && 'application' == get_post_type( (int) $_REQUEST['post_id'] ) ) {
			return true;
		}
	...
	public function edit_columns( $existing_columns ) {
		...
		$columns['dates']          = __( 'Proposed Start / End Dates', 'propertyhive' );
		$columns['rent']           = __( 'Proposed Rent', 'propertyhive' );
	...
	public function custom_columns( $column ) {
		global $post, $propertyhive, $the_application;

		if ( empty( $the_application ) || $the_application->ID != $post->ID )
		{
			$the_application = new PH_Application( $post->ID );
		}
		...
		switch ( $column ) {
			...
			case 'property' :
				...
				$property = new PH_Property( (int) $the_application->property_id );
			...
			case 'property_owner' :
				...
				$the_property      = new PH_Property( (int) $the_application->property_id );
			...
			case 'applicant' :
				...
				echo $the_application->get_tenants(false, true);

				break;
			case 'dates' :
				echo 'Start Date: ' . ( $the_application->_start_date != '' ? date( "d/m/Y", strtotime( $the_application->_start_date ) ) : '-' ) . '<br>';
				echo 'End Date: ' . ( $the_application->_end_date != '' ? date( "d/m/Y", strtotime( $the_application->_end_date ) ) : '-' );

				break;
			case 'rent' :

				echo $the_application->get_formatted_rent();

				break;
			case 'status' :

				echo $the_application->get_status();
...
return new PH_Admin_CPT_Application();

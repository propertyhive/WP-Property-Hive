...
class PH_Form_Handler {
	...
	public function __construct() {
		...
		foreach ( $manager->get_forms() as $id => $form ) {
			add_filter(
				'propertyhive_search_form_fields_' . $id,
				function( $fields ) use ( $manager ) {
					$form_id = str_replace( 'propertyhive_search_form_fields_', '', current_filter() );

					return $manager->apply_form_settings_to_fields( $fields, $form_id );
				},
				99,
				1
			);

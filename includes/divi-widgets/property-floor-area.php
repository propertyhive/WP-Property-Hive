...
class Divi_Property_Floor_Area_Widget extends ET_Builder_Module
{
    public $slug       = 'et_pb_property_floor_area_widget';
    public $vb_support = 'partial';

    public function init() {
        $this->name = esc_html__( 'Property Floor Area', 'propertyhive' );
        $this->icon = '&';
    ...
    public function render( $attrs, $content, $render_slug )
    {
        ...
        if ( $property->department == 'commercial' && ( ( $property->floor_area_to_sqft != '' && $property->floor_area_to_sqft != 0 ) || ( $property->floor_area_from_sqft != '' && $property->floor_area_from_sqft != 0 ) ) )
        {
            ...
            $return .= '<div class="divi-widget-floor-area">';

            $return .= $image;

            if ( isset($this->props['before']) && $this->props['before'] != '' ) { $return .= $this->props['before'] . ' '; }

            $return = $property->get_formatted_floor_area();
